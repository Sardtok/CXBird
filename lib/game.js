...
function crashDetect(p) {
    ...
    for (i = 0; i < bars.length; i++) {//bars are sorted after x value from left to right
        ...
        if (b.x > p.x + playerWidth) { //in front of player
            ...
            return false;
        } else if (p.x > b.x + b.w) {//behind player
            continue;
        } else if (p.y < b.hy1) {//possible hit upper bar
            if (p.y < b.hy1 - playerRad) {
                ...
                bcx = b.x + bRad; //bar1 center x
                bcy = b.hy1 - bRad; //bar 1 center y
                distSquared = (bcx - cx) * (bcx - cx) + (bcy - cy) * (bcy - cy);
                maxDist = bRad + playerRad;
                if (dist < maxDist * maxDist) { // yeah, dead
                    p.die();
                }
            }
            ...
            if (p.y + playerRad > b.hy2) {//yeah, no doubt
                ...
                cy = p.y + playerRad;//player center y
                bcx = b.x + bRad; //bar1 center x
                bcy = b.hy2 + bRad; //bar 1 center y
                distSquared = (bcx - cx) * (bcx - cx) + (bcy - cy) * (bcy - cy);
                maxDist = bRad + playerRad;
                if (distSquared < maxDist * maxDist) { // yeah, dead
                    p.die();
                }
            }
        }
